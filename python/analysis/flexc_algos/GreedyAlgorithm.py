...
class GreedyAlgorithm(FlexCAlgorithm.FlexCAlgorithm):
    ...
    def get_compartments_to_merge(db: HAKCDatabase):
        cmd = f"""
        MATCH 
        (comp1:{HAKCCompartment.get_table_name()})<-[:{HAKCDivision.relation_compartment}]-(div1:{HAKCDivision.get_table_name()})<-[:{HAKCSymbol.relation_division}]-(sym1:{HAKCSymbol.get_table_name()})-[dag:{HAKCSymbol.relation_dag}]->(sym2:{HAKCSymbol.get_table_name()})
