...
default, but this can be controlled by editing the
`cmake-build-hakc/linux/x86_64/hakc-server-config.yaml` and removing the `adjustment-path`
entry.
...
run the `hakc-linux-x86_64-analysis` target first.
...
`cmake-build-hakc/llinux/x86_64/hakc-config.yaml`). However, you can invoke the server and
provide the config and a list of files to compile, and the server will only compile the
provided files. Additionally, the server takes in a required mode, which is typically
either
