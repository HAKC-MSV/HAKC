...
namespace hakc {
    HAKCYamlSymbol::HAKCYamlSymbol()
            : Type(), Scope(), Name(), Definition(), CompartmentID(0), DivisionID(0), CompilationUnits() {

    }

    raw_ostream &operator<<(raw_ostream &os, HAKCYamlSymbol &YamlSymbol) {
        os << "HAKCSymbol(Name=" << YamlSymbol.Name << ", DbgType=" << YamlSymbol.Type.DebugType << ", LLVMType="
           << YamlSymbol.Type.LLVMType << ", Scope=";
