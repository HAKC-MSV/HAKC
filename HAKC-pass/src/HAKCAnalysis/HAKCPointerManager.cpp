...
namespace hakc {
    ...
    HAKCPointerManager::AddHAKCPointerReplacement(const ManagedHAKCPointerUseP &PtrUse, Value *Replacement,
        ...
        std::map<ManagedHAKCPointerUseP, Value *> &StorageToUse = (AddingAuthenticatedReplacements ? AuthenticatedValues
                                                                                                   : ProtectedValues);
        std::map<ManagedHAKCPointerUseP, Value *> &OtherStorage = (AddingAuthenticatedReplacements ? ProtectedValues
                                                                                                   : AuthenticatedValues);
        ...
        if (!ExistingPointer) {
            ...
            if (Replacement && ExistingPointer != Replacement) {
                CommonHAKCAnalysis::getWriter() << "Trying to replace existing " << StorageName << " Replacement " << ExistingPointer << " with " << Replacement << " for " << *PtrUse << "\n" << GetFunctionAnalysis()->getFunction() << "\n";
                throw std::exception();
            }
            if (Replacement) {
                if (DebugActive) {
